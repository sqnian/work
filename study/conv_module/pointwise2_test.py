...
def gen_data(batch_size, seq_len, feat_dim, odim):
    ...
    conv =  torch.nn.Conv2d(odim, odim, kernel_size=1,
            stride=1,
...
if __name__ == "__main__":
    ...
    start_time = time.time()
    for i in range(num_iter):
        (cuda_conv2) = conformer_infer_opt.test_conv(i_int8,i_int8.flatten(),w_int8_arr, w_int8_arr.flatten(),alpha_arr,bias,act_num,0)   
    torch.cuda.synchronize()
