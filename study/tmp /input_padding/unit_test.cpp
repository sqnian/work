...
)
{
    ...
    CUINFER_CHECK(cuinferSetActivationDescriptor(activation_descriptor,
                                        /*mode=*/  CUINFER_ACTIVATION_RELU,//CUINFER_ACTIVATION_IDENTITY,   // no activate 
...
at::Tensor input_padding(at::Tensor input);  // input shape : [batch_size, cin, seq_len, hidden_size]

at::Tensor weight_padding(at::Tensor weight); // weight shape : cout,,cin,ker-h,ker-w

at::Tensor output_data(at::Tensor output); // output : 32,176,19,256 : NHWC 

at::Tensor subsampling(
    at::Tensor inputs,
    at::Tensor weight_1,
    at::Tensor bias1,
    at::Tensor weight2,
    ...
    at::Tensor input = input_padding(inputs); //input  batch_size, seq_len, hidden_size, cin: 32,710,80,32
    TORCH_CHECK(input.dim() == 4);
    TORCH_CHECK(input.is_contiguous());
    TORCH_CHECK(input.is_cuda());
    TORCH_CHECK(input.scalar_type() == at::ScalarType::Half);

    at::Tensor weight1 = weight_padding(weight_1); // 
    TORCH_CHECK(weight1.dim() == 4);
    TORCH_CHECK(weight1.is_contiguous());
    TORCH_CHECK(weight1.is_cuda());
    TORCH_CHECK(weight1.scalar_type() == at::ScalarType::Half);
    ...
    conv2d_relu(n, c_out, h_out, w_out, c_out_2,
                pad_h_2,pad_w_2,stride_h_2,stride_w_2,kernel_h_2,kernel_w_2,h_out_2,w_out_2,
                cu_bias2,wei_ptr_2,out_ptr,out_ptr_2);
    at::Tensor res = output_data(output_2);
    return res;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
   // test conv2d fp16 api
    ...
    m.def("test_func_padding", &subsampling,"test input data padding double conv2d and relu fp16 api");
