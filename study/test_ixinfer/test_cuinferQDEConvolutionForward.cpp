...
void cpuQDEConvolutionForward(layer l,
                            const float* alpha,
                           const float* gamma,
                           const int8_t* input,
                           const int8_t* weight,
                          const float* bias,
                          const int8_t* element_input,
        ...
        for (int p = 0; p < P; ++p){
            for (int q = 0; q < Q; ++q){
                for (int k = 0; k < l.padded_outc; ++k) {
                    ...
                    tmp_output = (tmp_output+element_input[idx]*element_scale[0])*alpha2[0];
...
void run_cuinferQDEConvolutionForward(layer l,
...
int compare_qdeconv(int batch_size, int input_h, int input_w, int input_c, int kernel_h, int kernel_w, int  output_c, int output_h, int output_w, int input_pad, int input_stride, 
    ...
    int8_t *element_input = new int8_t[output_size];
    ...
    float alpha2 = 0.1 * static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
    float element_scale = 0.1 * static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
    ...
    cpuQDEConvolutionForward(cur_l, quant_alpha, &leaky_gamma, input_x,input_weight,input_bias,element_input, &element_scale, &alpha2,use_leaky,use_pechannel,cpu_output_y);
    run_cuinferQDEConvolutionForward(cur_l, input_x,input_weight,input_bias,quant_alpha,element_input, &element_scale, &alpha2,&leaky_gamma,cuinfer_output_y,group_num,&exec_time,
...
//int compare_qdeconv(int batch_size, int input_h, int input_w, int input_c, int kernel_h, int kernel_w, int  output_c, int output_h, int output_w, int input_pad, int input_stride, bool use_leaky,bool use_pechannel, bool use_bias,float leaky_gamma)
